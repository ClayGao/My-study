...
- Round 9，接續上一步， 繼續執行 inner EC
    ...
    global EC :{
        VO: {
            test : point to finction test,
            a: 1,     
            b: 200 // 因為 inner EC 中沒有 b，所以往上找 test EC，test EC 也沒有 b，再往上找 global EC，還是沒有 b，但已經沒有再外一層了，這時候底層只好在 global EC 創建一個 b，之後執行賦值 b = 200 結束
        }
    }
    ```
...
- Round 12，執行　`console.log('5.', a)`
    ...
    ```javascript
    console.log('4.', a)　// 找到 a 為 1，輸出 1 結束
...
- Round 14，執行　`console.log('6.', a)`
    ...
    ```javascript
    console.log('4.', a)　// 找到 a 為 70，輸出 70，移出
...
- Round 15，執行　`console.log('7.', a)`
    ...
    ```javascript
    console.log('4.', a)　// 找到 b 為 200，輸出 200，移出
